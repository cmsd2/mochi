...
- [`06_rl_maglev`](docs/notebooks/06_rl_maglev.md) — reinforcement learning a stabilising controller for an unstable plant. A magnetically levitated ball with $\propto i^2/x^2$ attractive force (one positive eigenvalue open-loop). Trains PD gains via cross-entropy method (`np_cem` from `numerics-learn`) against the *nonlinear* plant, side-by-side with closed-form pole placement against the linearised one. Closes the loop with a symbolic Maxima policy expression that mochi compiles to a native Lisp closure (no Maxima evaluator overhead per CVODE step).
- [`07_rl_two_tank`](docs/notebooks/07_rl_two_tank.md) — tabular Q-learning (`np_qlearn`) over a coupled-MIMO two-tank cascade with on/off pumps. Dual setpoints (h₁ = 0.10, h₂ = 0.20) make both pumps essential. Demonstrates the `mod_advance` bridge — a one-step continuous-state advance that lets discrete-time RL APIs drive a continuous-DAE plant — and ends with each pump cycling at ~42% duty (theory predicted 40%).
